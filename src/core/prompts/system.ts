...
import { getModesSection, addCustomInstructions, getCommandChainOperator, getCommandChainNote } from "./sections"
...
async function generatePrompt(
	...
	let skillsXml = ""
	if (skillsManager && mode) {
		const skills = skillsManager.getSkillsForMode(mode)
		if (skills.length) {
			skillsXml = skills
				.map((skill) => {
...
<context_notes>
- The skill list is already filtered for the current mode: "${mode}".
- Mode-specific skills may come from skills-${mode}/ with project-level overrides taking precedence over global skills.
</context_notes>
...
4. Once you've completed the user's task, you must present the result of the task to the user in a text response, and then call the attempt_completion tool as a marker.
